...
const validateUser = async () => {
  ...
  let str = `
      ...
      <p onclick='showData(5)'><i class="bi bi-stickies"></i> My Todos</p>
...
const getComments = async (postId) => {
  ...
  json.map((element) => {
    str += `<p><b>${element.name}:</b>
    <p><b>Title:</b>${element.email}
        <p><b>Body:</b>${element.body}
...
const getTodos = async () => {
  ...
  json.map((element) => {
    ...
    if(element.completed == true){
      ...
      str+=`<p><input type="checkbox" >${element.title}</p>`
    }
    // str += `
    // <p><b>Title:</b>${element.title}</p>
    // <hr>`;
